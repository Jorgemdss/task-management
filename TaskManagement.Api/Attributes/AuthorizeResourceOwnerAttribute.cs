...
public class AuthorizeResourceOwnerAttribute<TService>(
    string idParameterName = "id",
    string resourceName = "resource"
) : Attribute, IAsyncActionFilter
    where TService : IResourceOwnerService
{
    ...
    )
    {
        ...
        if (
            !context.ActionArguments.TryGetValue(idParameterName, out var entityId)
            || entityId is not Guid resourceId
        )
        {
        ...
        var auth = await authService.AuthorizeAsync(
            ...
            ownerId,
            new UserOwnedResourcePermission()
        );

        if (!auth.Succeeded)
        {
            context.Result = new ObjectResult(
                new { message = $"You don't have access to the {resourceName} {resourceId}." }
            )
            {
